...
# path是访问路径,当访问path(/static)开头的路径时,会找directory(static)目录下的文件,并返回给客户端
...
# 数据模型
class ApiResponse(BaseModel):  # BaseModel是pydantic模块中的一个类,用于定义数据结构
    code: int  # FastAPI 自动解析请求体，将 session_id 和 message 赋值到 ChatRequest 对象的对应字段
    message: str
    data: Any  # 任意类型
...
def root():
    logging.info("访问项目首页")
    return FileResponse("static/index.html")  # 把HTML文件作为响应返回给客户端,浏览器端解析HTML文件并显示
...
def create_session() -> ApiResponse:
    ...
    # FastAPI会对返回的类型进行校验,校验对象中的属性符不符合要求,并使用pydantic模块自动将数据序列化为JSON格式
...
def chat(request: ChatRequest) -> ApiResponse:
    ...
    logging.info(f"<---- 响应的数据: {ai_response}")

    # 5. 更新消息列表中的消息
    messages.pop(0)  # 删除第一个元素(系统提示词)
...
def get_sessions() -> ApiResponse:
    logging.info("获取会话列表")
    # 1. 获取sessions目录下所有会话文件名
    session_files = os.listdir("sessions")

    # 2. 获取会话列表(去掉.json)
    session_ids = [file.split(".")[0] for file in session_files]  # 按.切割为两个元素，取第一个元素
...
# 获取指定的会话信息
@app.get("/api/sessions/{session_id}")  # session_id是路径参数,起到通配的作用,进行路径匹配
...
# 定义异常处理器,捕获所有异常 --> 返回的对象类型是 Response , 这里的JSONResponse是Response的子类
@app.exception_handler(Exception)
def handle_exception(request: Request, exc: Exception):
    # request: 发送哪个请求的时候报错了 ; exc: 捕获到的异常信息
...
if __name__ == '__main__':
    ...
    # access_log: 是否打印访问日志(默认为true,不建议设置为false)
